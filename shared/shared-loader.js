...
(function () {
  ...
  function parseKategoriPetugasKota(rows) {
    ...
    const out = rows.map(r => {
      ...
      const pctSelesai = pickField(r, ["% Selesai", "%Selesai", "Persentase Selesai", "Pct Selesai"]);
  ...
  function parseHeatmapKecKategori(rows) {
    ...
    const out = rows.map(r => {
      const kecamatan = pickField(r, ["Kecamatan", "Nama Kecamatan"]);
  ...
  window.SheetEngine = {
    ...
    rowsToObjects, rowsToKV, parseCSV, toNum, fmtID, yesNo, pct
  };
